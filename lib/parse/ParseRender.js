...
module.exports = {
  ...
  processNodes (file, data, document) {
    ...
    // after component instances because there we set the component properties
  ...
  replaceLinks (document) {
    for (const node of document.querySelectorAll('a[href]')) {
      const href = node.getAttributeNS(null, 'href')
      if (href.startsWith('http') || href.startsWith('#') || href.startsWith('mailto:') ||
        href.startsWith('tel:')) {
        continue
      }
      node.removeAttributeNS(null, 'href')
      node.setAttributeNS(null, 'to', href)
      Html.changeTag(node, 'Link', document)
    }
  },

  replaceAttributes (document) {
    const camelMap = ParseCommon.getCamelCaseAttributeMap()
    const shorts = ParseCommon.getShortAttributes()
    for (const node of document.querySelectorAll('*')) {
      // skip svg children elements
      if (node.closest('svg')) continue
      for (const [name, value] of Object.entries(Html.getAttributes(node))) {
        this.replaceAttribute(node, name, value, camelMap, shorts)
      }
    }
  },

  replaceAttribute (node, name, value, camelMap, shorts) {
    ...
    } else if (shorts.includes(name)) {
      node.setAttributeNS(null, name, name)
    } else if (name === 'selected' && Html.getTag(node) === 'option') {
