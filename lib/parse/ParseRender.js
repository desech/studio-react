...
module.exports = {
  ...
  injectComponentRootData (root, file, folder) {
    ...
    root.setAttributeNS(null, 'className', `${classes} ${cssClass} desech-regex-root-class`)
  ...
  injectIfForNode (node, props, document) {
    if ('reactIf' in props) {
      Html.wrapNode(node, 'desechIf', document, { value: props.reactIf })
    } else if ('reactFor' in props) {
      Html.wrapNode(node, 'desechFor', document, { value: props.reactFor })
    } else if ('reactIfFor' in props) {
      Html.wrapNode(node, 'desechIfFor', document, { value: props.reactIfFor })
    } else if ('reactForIf' in props) {
      Html.wrapNode(node, 'desechForIf', document, { value: props.reactForIf })
    }
  },
