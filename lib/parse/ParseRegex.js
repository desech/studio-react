...
module.exports = {
  ...
  regexHtmlRender (file, data, componentObj, document, lib) {
    ...
    html = html.replace(/<(img|input|track|hr|br)(.*?)>/g, '<$1$2 />')
    html = this.replaceAttributes(html)
    html = this.addElementProperties(html, data, componentObj)
    html = this.addComponentProperties(html, data, componentObj)
  ...
  replaceLinks (html) {
    ...
    return html.replace(regex, (match, extra1, url, extra2, text) => {
      return url.startsWith('http')
        ...
        : `<Link${extra1}to="${url}"${extra2}>${text}</Link>`
    })
  },
  ...
  addElementProperties (html, data, componentObj) {
  ...
  addComponentProperties (html, data, componentObj) {
    // we can't add attributes with setAttributeNS because we allow invalid html/xml attributes
