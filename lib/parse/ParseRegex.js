...
module.exports = {
  ...
  addProperties (html, data, componentObj) {
    return html.replace(/data-ss-properties="(.*?)"/g, (match, string) => {
      const ref = string.substring(0, 8) // refs are 8 chars in length
      const json = string.substring(9) // skip the pipe
