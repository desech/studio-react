...
module.exports = {
  ...
  parseClassJsCode (js, file, relBase, data, lib) {
    ...
    const component = this.getComponentData(body, render.component, overrideImports)
    js = this.injectJs(js, component, 'data', 4)
    const importCode = this.getCodeImport(body, render.html, relBase, data, overrideImports)
    js = this.injectJs(js, importCode, 'import')
    const propTypes = this.getPropTypes(render.component)
    js = this.injectJs(js, propTypes, 'types', 2)
    return js
  },
  ...
  getCodeImport (body, render, relBase, data, overrideImports) {
    const list = []
    this.addLibImports(list, render, relBase)
  ...
  addLibImports (list, render, relBase) {
    if (render.indexOf('</Link>') >= 0) {
