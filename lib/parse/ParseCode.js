...
module.exports = {
  ...
  parseClassJsCode (js, file, relBase, data, lib) {
    ...
    const imports = this.getComponentImports(components, relBase, overrideImports)
    js = this.injectJs(js, imports, 'import')
    js = this.injectJs(js, this.getComponentPropTypes(render.component), 'types', 2)
    return js
  },
  ...
  getComponentImports (components, relBase, overrideImports) {
    const list = []
    list.push(`import DS from '${File.relative(relBase, '/lib/DS.js')}'`)
    this.addComponentImports(list, components, relBase)
    this.addComponentImports(list, overrideImports, relBase)
    return ExtendJS.unique(list).join('\n') + '\n'
  },

  addComponentImports (list, components, relBase) {
