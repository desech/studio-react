...
module.exports = {
  ...
  getClassJsFile (folder, filePath, fileType) {
    filePath = (fileType === 'component') ? filePath.replace('/component', '') : filePath
    const dir = ParseCommon.getClassDir(filePath, folder)
    const cls = ParseCommon.getClassName(File.basename(filePath))
    return File.resolve(folder, `_export/src/${fileType}${dir}/${cls}.js`)
  },

  parseClassJsCode (js, file, relBase, fileType, css, lib) {
    const body = this.getClassBody(file.path)
    const rendered = ParseRender.getRenderBlock(body, css, fileType, lib)
  ...
  setClassComponentImports (list, body, relBase) {
    ...
    for (const match of matches) {
      ...
      const line = this.getClassComponentImport(json.file, relBase)
      list.push(line)
    }
  },
  ...
  getClassComponentImport (htmlFile, relBase) {
    ...
    const relFile = `/${ParseCommon.getClassDir(htmlFile)}/${component}.js`
