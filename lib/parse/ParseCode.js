...
module.exports = {
  ...
  parseClassJsCode (js, file, relBase, data, lib) {
    ...
    const importCode = this.getCodeImport(body, relBase, data, overrideImports)
  ...
  addInstance (data, value, overrideImports) {
    if (!value.startsWith('component/')) return
    ...
    data.component = '__INST__' + ParseCommon.getClassName(value) + '__INST__'
  },

  getCodeImport (body, relBase, data, overrideImports) {
    const list = []
