...
module.exports = {
  ...
  getAttributeCode (ref, name, value, type, exists, filter = false) {
    ...
    const indexDel = ref + 'Attrdel' + ExtendJS.toPascalCase(name)
    if (filter) name = name.replace(/[^a-zA-Z0-9-_]/g, '')
    value = Html.escapeQuotedHtml(value, "'")
    if (type === 'update' && !exists) {
      return '{...DS.e(d.' + index + ') && {\'' + name + '\':d.' + index + '}}'
    } else if (type === 'update') {
      return name + '={DS.e(d.' + index + ') ? d.' + index + ' : \'' + value + '\'}'
    } else if (type === 'delete') {
      return '{...!d.' + indexDel + ' && {\'' + name + '\':\'' + value + '\'}}'
    } else if (type === 'update-delete') {
      return '{...!d.' + indexDel + ' && {\'' + name + '\': DS.e(d.' + index +
        ') ? d.' + index + ' : \'' + value + '\'}}'
    }
  },
