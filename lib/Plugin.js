...
module.exports = {
  ...
  getAllComponentData (data) {
    ...
    for (const file of data.htmlFiles) {
      ...
      this.extractComponentData(html, file.path, data.folder, obj)
    }
    return obj
  },

  extractComponentData (html, file, folder, obj) {
    html.matchAll(/data-ss-component="(.*?)"/g, (match, json) => {
      const data = JSON.parse(json.replaceAll('&quot;', '"'))
      const cmpFile = data.file || file.replace(folder, '')
      obj[cmpFile] = ''
    })
  },
