...
module.exports = {
  async syncAppFiles (folder) {
    const source = File.normalize(__dirname + '/../dist/my-app') // eslint-disable-line
  ...
  async syncStaticFolders (data) {
    const dir = data.folder + '/_export/public'
    const file = dir + '/css/compiled/style.css'
  ...
  syncJsModule (folder, file, type, JSDOM) {
    ...
    const relBase = destFile.replace(folder + '/_export/src', '')
