...
module.exports = {
  ...
  syncJsCode (data, lib) {
    ...
    ParseCommon.replaceAllDataAttributes(data.component.overrides)
  ...
  syncJsComponents (data, lib) {
    for (const file of data.htmlFiles) {
      if (file.isComponent) {
        const success = this.syncJsModule(data, file, lib)
        if (success) this.syncComponentStory(data, file)
      }
    }
  },
