...
module.exports = {
  ...
  syncLibs (folder, lib) {
    const src = File.resolve(__dirname, 'template', 'DS.js')
    const dest = File.resolve(folder, '_export/src/lib/DS.js')
  ...
  syncJsComponents (data, lib) {
    for (const file of data.htmlFiles) {
      if (file.isComponent) {
        this.syncJsModule(data, file, 'component', lib)
      }
    }
  },
  ...
  syncJsModule (data, file, fileType, lib) {
    ...
    js = ParseCode.parseClassJsCode(js, file, relBase, fileType, data, lib)
