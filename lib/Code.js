...
module.exports = {
  ...
  injectIfFor (document) {
    for (const tag of document.querySelectorAll('[data-programming-properties]')) {
      ...
      if ('reactIf' in props) {
        ...
        this.injectIfForTag('iffor', document, tag, props.reactIfFor.split('/'))
      } else if ('reactForIf' in props) {
        this.injectIfForTag('forif', document, tag, props.reactForIf.split('/'))
      }
  ...
  injectIfForTag (type, document, tag, value) {
    ...
    this.injectIfForAttributes(type, wrapper, value)
    tag.parentNode.insertBefore(wrapper, tag)
    wrapper.appendChild(tag)
  },

  injectIfForAttributes (type, wrapper, value) {
    if (type === 'iffor') {
      wrapper.setAttributeNS(null, 'data-if', value[0])
      wrapper.setAttributeNS(null, 'data-loop', value[1])
      wrapper.setAttributeNS(null, 'data-map', value[2])
    } else if (type === 'forif') {
      wrapper.setAttributeNS(null, 'data-loop', value[0])
      wrapper.setAttributeNS(null, 'data-map', value[1])
      wrapper.setAttributeNS(null, 'data-if', value[2])
    } else {
  ...
  replaceForLoop (html) {
    ...
    return html.replace(/<for value="(.*?)">/g, (match, value) => {
      const loop = value.split('/')
      return `{${loop[0]}.map(${loop[1]} => `
    }).replace(/<\/for>/g, ')}')
  },
  ...
  replaceIfForCombo (html) {
    ...
    return html.replace(/<iffor data-if="(.*?)" data-loop="(.*?)" data-map="(.*?)">/g,
      (match, ifVal, loopVal, mapVal) => {
        return `{${ifVal} && ${loopVal}.map(${mapVal} => `
      }).replace(/<\/iffor>/g, ')}')
  },
  ...
  replaceForIfCombo (html) {
    ...
    return html.replace(/<forif data-loop="(.*?)" data-map="(.*?)" data-if="(.*?)">/g,
      (match, loopVal, mapVal, ifVal) => {
        return `{${loopVal}.map(${mapVal} => ${ifVal} && `
      }).replace(/<\/forif>/g, ')}')
  }
}
