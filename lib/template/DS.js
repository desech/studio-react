...
export default class DS {
  ...
  static buildDataVariant (variants) {
    ...
    // if no variants it will show data-variant="foo-var= foo2= foo3="
  ...
  static setDefaults (defaults, data) {
    ...
    for (const [ref, object] of Object.entries(defaults)) {
      for (const [name, value] of Object.entries(object)) {
        // name = tag, component, inner, unrender, attributes, classes, overrides, variants
        this.setDefaultValue(data, ref, name, value)
      }
    }
  }

  static setDefaultValue (data, ref, name, value) {
    switch (name) {
      case 'tag': case 'component': case 'inner': case 'unrender':
        ...
        break
      case 'attributes':
        this.setAttributesValue(data, ref, this.convertAttributes(value))
        break
      case 'classes':
        this.setClassesValue(data, ref, this.convertClasses(value))
        break
      case 'overrides':
  ...
  static setOverrides (overrides, data) {
    ...
    for (const [ref, obj] of Object.entries(overrides)) {
      for (const [name, value] of Object.entries(obj)) {
        this.setOverrideValue(data, ref, name, value)
      }
    }
  }

  static setOverrideValue (data, ref, name, value) {
    switch (name) {
  ...
  // the default object has the attributes in this format {foo: 'bar', foo2: 'bar2'}
  // convert it to the object format of overrides: {foo: {value: 'bar'}, foo2: {value: 'bar2'}}
  ...
  // the defaults object has the classes in an array format: ['red', 'blue']
  // convert it to the object format of overrides: {red: {value: 'red'}, blue: {value: 'blue'}}
  static convertClasses (classes) {
    const obj = {}
